...
import type {
  FetchInpiDataTool,
  FilterInpiResultsTool,
  FindMostRecentTrademarkTool,
  GetInpiDetailsTool,
} from "../../baml_client"; // Import BAML tool types
...
export class InpiFilteredResults implements LLMSerializable {
  ...
  constructor(
    public trademarks: TrademarkEntry[],
  ...
  toLLMString(): string {
    ...
    if (this.trademarks.length > 0) {
      ...
      for (const t of this.trademarks.slice(0, 5)) {
        // Show more details for filtered results
        summary.push(
          `- Numero: ${t.Numero ?? "N/A"}, Marca: ${t.Marca ?? "N/A"}, Situacao: ${t.Situacao ?? "N/A"}, Titular: ${t.Titular ?? "N/A"}`
        );
      }
...
export class TrademarkEntryResult implements LLMSerializable {
  ...
  toLLMString(): string {
    ...
    return (
      `Details for Numero ${entry.Numero}:\n` +
...
export type InpiHandlerResult =
  | InpiFetchSummary
  | InpiFilteredResults
  | TrademarkEntryResult
  | InpiErrorResult;
...
async function scrapeAllInpiData(marca: string): Promise<InpiRawResults> {
  ...
  try {
    ...
    let response = await client.post(searchUrl, searchData.toString(), {
      headers,
    });
    ...
    while (currentPage <= totalPages) {
      ...
      if (currentPage > 1) {
        const pageData = new URLSearchParams({
          Action: "nextPageMarca",
          page: currentPage.toString(),
        });
        try {
          response = await client.post(searchUrl, pageData.toString(), {
            headers,
          });
          content = response.data as string;
        } catch (pageError: any) {
          errors.push(
            `Failed to fetch page ${currentPage}: ${pageError.message || pageError}`
          );
      ...
      const tableRowsPattern =
        /<tr bgColor="#E0E0E0" class=normal>[\s\S]*?<\/tr>|<tr bgColor="white" class=normal>[\s\S]*?<\/tr>/g;
      ...
      if (rows) {
        ...
        for (const rowHtml of rows) {
          ...
          if (urlMatch)
            url = urlMatch[1].trim().startsWith("/")
              ? `https://busca.inpi.gov.br${urlMatch[1].trim()}`
              : urlMatch[1].trim();
          numero = (numeroLinkMatch ? numeroLinkMatch[1] : columns[0])
            .replace(/<[^>]+>/g, "")
            .trim();
      ...
      if (currentPage < totalPages && !content.includes(">Próxima»</a>")) {
        console.warn(
          `'Próxima' link not found on page ${currentPage} despite totalPages being ${totalPages}. Stopping pagination.`
        );
    ...
    errors.push(
      `Request failed during search/pagination: ${error.message || error}`
    );
  }
...
function parsePrioridadeDate(
  ...
  if (
    ...
    console.warn(
      `Invalid date components after parsing: ${dateString} -> D:${day}, M:${month + 1}, Y:${year}`
    );
    return null;
  }
  ...
  if (
    ...
    console.warn(
      `Date components rolled over during Date construction: ${dateString}`
    );
    return null;
  }
  return date;
 ...
 */
async function fetchInpiDataHandler(
  step: FetchInpiDataTool
): Promise<InpiFetchSummary> {
  if (getLogLevel() !== "OFF")
  ...
  if (getLogLevel() !== "OFF")
    console.log(
      `INPI fetch complete for ${step.marca}. Result ID: ${resultId}, Summary: ${summary}`
    );
 ...
 */
async function filterInpiResultsHandler(
  step: FilterInpiResultsTool
): Promise<InpiFilteredResults | InpiErrorResult> {
  if (getLogLevel() !== "OFF")
    console.log(
      `Filtering INPI results for ID: ${step.result_id} with criteria:`,
      {
  ...
  const filtered = cachedResults.filter((t) => {
    ...
    if (
      step.situacao &&
      t.Situacao?.toLowerCase() !== step.situacao.toLowerCase()
    ) {
      match = false;
    }
    if (
      step.titular &&
      !t.Titular?.toLowerCase().includes(step.titular.toLowerCase())
    ) {
      match = false;
    }
    ...
    if (
      step.classe_ncl &&
      !t.Classes?.some(
        (c) => c.toLowerCase() === step.classe_ncl?.toLowerCase()
      )
    ) {
      match = false;
    }
  ...
  if (getLogLevel() !== "OFF")
    console.log(
      `Filtering complete for ID: ${step.result_id}. Found ${filtered.length} matching trademarks.`
    );
 ...
 */
async function getInpiDetailsHandler(
  step: GetInpiDetailsTool
): Promise<TrademarkEntryResult | InpiErrorResult> {
  if (getLogLevel() !== "OFF")
    console.log(
      `Getting INPI details for ID: ${step.result_id}, Numero: ${step.numero}`
    );
  const cachedResults = ThreadStore.getInpiResults(step.result_id);
  ...
  if (getLogLevel() !== "OFF")
    console.log(
      `Details found for Numero ${step.numero} in results ID: ${step.result_id}.`
    );
 ...
 */
async function findMostRecentTrademarkHandler(
  step: FindMostRecentTrademarkTool
): Promise<TrademarkEntryResult | InpiErrorResult> {
  if (getLogLevel() !== "OFF")
    console.log(
      `Finding most recent trademark for result ID: ${step.result_id}`
    );
  const cachedResults = ThreadStore.getInpiResults(step.result_id);
  ...
  if (cachedResults.length === 0) {
    const errorMsg =
      "No trademarks found in the results to determine the most recent.";
  ...
  for (const entry of cachedResults) {
    ...
    if (currentDate) {
      if (!maxDate || currentDate.getTime() > maxDate.getTime()) {
        // Compare time for precision
        maxDate = currentDate;
        mostRecentEntry = entry;
      }
    } else if (getLogLevel() !== "OFF")
      console.warn(
        `Skipping entry with unparseable date: ${entry.Numero} - ${entry.Prioridade}`
      );
  }
  ...
  if (getLogLevel() !== "OFF")
    console.log(
      `Most recent trademark found for ID ${step.result_id}: Numero ${mostRecentEntry.Numero} with date ${mostRecentEntry.Prioridade}`
    );
